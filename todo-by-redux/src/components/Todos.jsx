    ...
    return (
        <>
            ...
            <ul className='ps-0 m-0'>
                {todos.map((todo) => (
                    <li className=' list list-unstyled class  p-2 mt-1  d-flex justify-content-between rounded-3 h5 text-white' key={todo.id}>{todo.text}
                        <button onClick={() => dispatch(removeTodo(todo.id))}className=' text-center d-flex align-items-center delete border-1 p-1 rounded-2 shadow-sm'>
                            <svg className='' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" color="white" fill="none">
